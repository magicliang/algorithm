...
public class InPlaceMergeSort {
    ...
    private static void inPlaceMerge(int[] arr, int[] temp, int left, int mid, int right) {
        ...
        // 【中转机制核心】：原地内存操作的核心思想是：准备一个和现空间一样大的空间，先把数据拷贝过去，然后再拷贝回这个共享数据段
        // 这个临时数组被所有递归层级复用，通过局部映射避免了每次递归创建新数组的O(n log n)空间开销
        // 
        // 【类比汉诺塔】：就像汉诺塔需要中间柱子作为辅助，归并排序需要辅助数组作为"中转站"
        // - 汉诺塔：盘子从源柱 → 辅助柱 → 目标柱
        // - 归并排序：数据从原数组 → 辅助数组 → 原数组（排序后）


        // 【中转机制第一步：临时转移】
        // 将待排序区间复制到辅助数组，为安全归并做准备
        // 【索引映射技巧】：如果数组完全和原始 arr 总长相等，则可以一对一映射
        // 否则，这里面的 temp 要从0开始，而 arr要从 left 开始，实现局部区间的数据映射
        // 这一步相当于汉诺塔中将盘子从源柱移动到辅助柱的过程
        if (right + 1 - left >= 0) {
            System.arraycopy(arr, left, temp, 0, right + 1 - left);
        }

        // 【中转机制第二步：安全归并】
        // 设置两个游标，分别在区间的起点，在辅助数组上进行双指针归并
        // 这一步相当于汉诺塔中在辅助柱上重新组织盘子的过程
        ...
        // 【中转机制第三步：写回原位】
        // 将排序结果从辅助数组写回原数组对应区间
        // 这一步相当于汉诺塔中将排序好的盘子从辅助柱移回目标柱的过程
        while (i <= tempMid && j <= end) {
            if (temp[i] <= temp[j]) {
                arr[k++] = temp[i++];  // 从辅助数组写回原数组
            } else {
                arr[k++] = temp[j++];  // 从辅助数组写回原数组
            }
        }

        // 处理剩余元素：继续从辅助数组写回原数组，完成中转过程
        while (i <= tempMid) {
            arr[k++] = temp[i++];  // 左子数组剩余元素写回
        }

        while (j <= end) {
            arr[k++] = temp[j++];  // 右子数组剩余元素写回
        }
        
        // 【中转机制完成】：至此，原数组的[left, right]区间已完成排序
        // 整个过程体现了"借助辅助结构完成复杂操作"的设计思想
    }
}