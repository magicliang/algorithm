...
**核心洞察**：DP的每个迭代步骤，本质上就是在执行DFS在对应状态下的决策逻辑。
...
int dfs(int i, int capacity) {
    ...
    // 核心决策：选择还是不选择当前物品
    int choice1 = dfs(i-1, capacity);                    // 不选
    int choice2 = dfs(i-1, capacity-weight[i]) + value[i]; // 选择
    
    return Math.max(choice1, choice2); // 选择最优方案
}
```

**DP迭代的局部过程**：
```java
// 在每个dp[i][j]的计算中，DP做的事情
for (int i = 1; i <= n; i++) {
    for (int j = 1; j <= capacity; j++) {
        // 完全相同的决策逻辑！
        int choice1 = dp[i-1][j];                    // 不选
        int choice2 = dp[i-1][j-weight[i]] + value[i]; // 选择
        
        dp[i][j] = Math.max(choice1, choice2); // 选择最优方案
    }
}
```
...
| 方面 | DFS | DP |
|------|-----|-----|
| **计算方向** | 自顶向下（Top-Down） | 自底向上（Bottom-Up） |
| **问题分解** | 从大问题递归到小问题 | 从小问题迭代到大问题 |
| **决策逻辑** | **完全相同** | **完全相同** |
| **状态依赖** | 依赖子问题的返回值 | 依赖已计算的DP表项 |

#### 5.3 统一的认知框架

```java
// 通用的状态转移模式
当前状态的最优解 = 在所有可能的决策中选择最优的一个

// DFS表达方式
return max(decision1_result, decision2_result, ...);

// DP表达方式  
dp[i][j] = max(decision1_result, decision2_result, ...);
```

**深层认知**：每个`dp[i][j]`的计算过程，本质上就是在模拟DFS在状态`(i,j)`时的决策过程。

**为什么DP更高效？**
- **DFS的问题**：重复计算，同一个子问题`dfs(i,j)`可能被多次调用
- **DP的优势**：每个子问题只计算一次，`dp[i][j]`只计算一次
- **本质上**：DP就是把DFS的递归调用树"拍平"成一个表格，按照依赖关系的顺序填表

#### 5.4 记忆口诀

> **"DP迭代 ≈ DFS的局部决策过程"**
> - **DFS**：从顶层问题开始，递归地分解到子问题
> - **DP**：从底层子问题开始，迭代地构建到顶层问题
> - **但在每个具体的状态转移点上，两者的决策逻辑是完全一致的**

这个认知对理解动态规划非常重要，它揭示了DP和递归之间的深层联系，也解释了为什么我们可以将递归解法机械地转换为DP解法。
