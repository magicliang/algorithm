 ...
 * 【状态转移方程的深度认知】：
 * 
 * **核心问题**：为什么完全背包选择物品后的子问题是 dp[i][c-w[i]] 而不是 dp[i-1][c-w[i]]？
 * 
 * **问题分解的本质**：
 * - 原问题：用前i种物品，在容量c下获得最大价值
 * - 选择第i种物品后的新问题：用前i种物品，在容量(c-w[i])下获得最大价值
 * - 这个新问题的解，加上当前物品的价值，就是选择当前物品的总价值
 * 
 * **为什么是dp[i]而不是dp[i-1]？**
 * - dp[i][c-w[i]]：选择当前物品后，仍然可以继续使用这种物品（完全背包）
 * - dp[i-1][c-w[i]]：选择当前物品后，不能再使用这种物品（01背包）
 * 
 * **数学递推的逻辑**：
 * ```
 * dp[i][c] = max(dp[i-1][c], dp[i][c-w[i]] + v[i])
 *                ↑不选择      ↑选择当前物品后的子问题 + 当前物品价值
 *                当前物品      
 * ```
 * 
 * **关键洞察**：
 * 当我们选择了一个物品，问题规模确实缩小了（从"容量c"变成"容量c-w[i]"），
 * 这个缩小后的问题的最优解，加上当前物品的价值，就是选择当前物品的总价值。
 * 由于物品可以重复使用，所以子问题仍然是在第i层决策空间内。
 * 
 * **适用范围**：
 * 这种 dp[i][c-cost] 的模式适用于所有"无限制重复选择"的问题：
 * - 完全背包问题（物品可重复选择）
 * - 硬币兑换问题（硬币可重复使用）
 * - 爬楼梯问题（每种步长可重复使用）
 *
