 ...
 * ↑不选择      ↑选择当前物品后的子问题（注意是dp[i-1]）
 * ```
 *
 * **完全背包状态转移**：
 * ```
 * dp[i][c] = max(dp[i-1][c], dp[i][c-w[i]] + v[i])
 * ↑不选择      ↑选择当前物品后的子问题（注意是dp[i]）
 * ```
 *
 * **关键差异解析**：
 * - **01背包**：dp[i-1][c-w[i]] - 选择物品i后，不能再选择物品i，所以是i-1层
 * - **完全背包**：dp[i][c-w[i]] - 选择物品i后，仍可以选择物品i，所以还是i层
 *
 * **问题本质**：
 * - **01背包**：每个物品最多选择一次，体现了"资源稀缺性"
 * - **完全背包**：每个物品可以选择多次，体现了"资源无限性"
 *
 * **子问题的含义**：
 * - **01背包的dp[i-1][c-w[i]]**：在已经"消耗"了物品i的前提下，用前i-1种物品在剩余容量下的最优解
 * - **完全背包的dp[i][c-w[i]]**：在已经选择了一个物品i的前提下，用前i种物品（包括可以继续选择物品i）在剩余容量下的最优解
 *
 * 这种差异直接决定了两种背包问题的不同性质和应用场景。
 *
 * ==================== 算法模板总结 ====================
 *
 * 1. 普通递归搜索模板：
...
public class ZeroOrOneKnapsacks {
    ...
    private int knapsacksMemoization(int[] weights, int[] values, int item, int capacity, int[][] memo) {
        ...
        if (weights[item - 1] > capacity) {
            ...
            int yes = knapsacksMemoization(weights, values, item - 1, capacity - weights[item - 1], memo) + values[item
                    - 1];
